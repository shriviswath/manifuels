-- ═══════════════════════════════════════════════════════════════════
-- ManiFuels — apply_all.sql   (v2.1.2)
--
-- Migrations 001–005 in order. Your base schema already exists and is NOT
-- recreated here.
--
...
--   -- expect 4 rows
--   select column_name from information_schema.columns
--   where table_name='app_settings' and column_name='config';   -- expect 1 row
--   select count(*) from dip_readings;                          -- expect 0
--   select tablename, rowsecurity from pg_tables
--   where schemaname='public' order by 1;                       -- all false
-- ═══════════════════════════════════════════════════════════════════
